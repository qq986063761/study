...
## 闭包
- 子函数内部保留父函数环境变量的方式；如：IIFE（立即执行函数）
- 作用：避免变量污染，可存储值（内部函数可访问外部函数内的变量，外部变量会一直被存储）
- 弊端：可能造成内存泄漏（老浏览器垃圾清理机制认为变量在使用中，则不会释放内存）
...
## 内存管理（GC：垃圾回收）
- 概念：js 创建变量时分配内存，不使用变量时“自动”释放内存（垃圾回收）
- 内存生命周期
  - 分配需要的内存
  - 使用分配到的内存（读、写）
  - 不需要时释放
- 垃圾回收机制
	...
	- 标记-清除算法（现代浏览器）：从根域开始，向下搜索所有的对象，将使用中的对象进行标记，未使用的对象进行清除；缺点：容易产生内存碎片
	- 新生代算法（Scavenge GC 算法）：将内存空间分为 from 和 to 两部分，新分配的对象被放入 from 中，当 from 空间占满时，检查 from 空间中被使用的对象复制到 to 空间，未被使用的对象进行销毁，然后再互换 to 和 from 空间
- 导致内存泄漏的因素：
	- 未置空的全局变量
	- 未释放的定时器
	- 未销毁的事件
