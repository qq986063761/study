...
import { ChevronRight, ChevronDown, User, Building2 } from "lucide-react"
import { cn } from "@/lib/utils"
import type { DepartmentHierarchyData } from "@/lib/types"

interface DepartmentViewProps {
  data: DepartmentHierarchyData[]
}
...
export function DepartmentView({ data }: DepartmentViewProps) {
  ...
  const expandAll = () => {
    ...
    const collectIds = (items: DepartmentHierarchyData[]) => {
      items.forEach((item) => {
  ...
  const summary = data.reduce(
    (acc, dept) => ({
      memberCount: acc.memberCount + dept.memberCount,
      totalActualHours: acc.totalActualHours + dept.totalActualHours,
      totalStandardHours: acc.totalStandardHours + dept.totalStandardHours,
    }),
    ...
    <div className="space-y-4">
      <Card className="bg-card border-border">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          <CardTitle className="text-base font-medium text-foreground">部门工时明细</CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border hover:bg-transparent">
                <TableHead className="text-muted-foreground min-w-[240px]">部门/人员</TableHead>
                <TableHead className="text-muted-foreground text-right">部门人数</TableHead>
