...
- `./global-components`：导出弹窗类组件对象，供 `ui.app1/ui.app2` 方法式调用
...
全局组件通过 `src/exports/global-components.ts` 暴露：
...
- 必须暴露 `./routes`、`./store`、`./ajax`、`./components`、`./global-components`
- routes 不写主应用挂载前缀
- ajax 只写相对接口路径
- store 不写 `namespaced: true`
- `./components` 只放局部组件加载器
- `./global-components` 只放弹窗类异步组件加载器，key 要稳定，例如 `modal`
