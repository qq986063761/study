...
const title = ref('app1 弹窗')
const content = ref('这是 app1 暴露给 main 的全局弹窗')
...
function show(options: ModalShowOptions = {}): void {
  title.value = options.title || 'app1 弹窗'
  content.value = options.content || options.message || '这是 app1 暴露给 main 的全局弹窗'
