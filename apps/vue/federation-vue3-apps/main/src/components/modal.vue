...
const title = ref('main 弹窗')
const content = ref('这是 main 自己的全局弹窗')
...
function show(options: ModalShowOptions = {}): void {
  title.value = options.title || 'main 弹窗'
  content.value = options.content || options.message || '这是 main 自己的全局弹窗'
...
<template>
  <el-dialog v-model="visible" :title="title" width="420px" append-to-body>
    ...
    <template #footer>
      <el-button type="primary" size="small" @click="hide">关闭 main 弹窗</el-button>
    </template>
  </el-dialog>
</template>
